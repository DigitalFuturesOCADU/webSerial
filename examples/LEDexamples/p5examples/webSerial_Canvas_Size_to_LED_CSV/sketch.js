/*
Example 3: Canvas Dimensions to WebSerial

This sketch demonstrates sending canvas dimensions over WebSerial.
It maps the canvas width and height to a 0-255 range and visualizes
...
- convertedX, convertedY: Mapped values of canvas width and height (0-255)

Key functions:
- updateConvertedValues(): Maps canvas dimensions to 0-255 range
...
function updateConvertedValues() {
  // Map canvas dimensions to 0-255 range
  convertedX = map(width, 0, displayWidth, 0, 255);
  convertedY = map(height, 0, displayHeight, 0, 255);
}
...
function displayText() {
  // Display current canvas dimensions and converted values
  fill(0);
  noStroke();
  textAlign(LEFT, TOP);
  textSize(16);
  text(`Canvas Width: ${width} px (${round(convertedX)})`, 10, 10);
  text(`Canvas Height: ${height} px (${round(convertedY)})`, 10, 30);
}
